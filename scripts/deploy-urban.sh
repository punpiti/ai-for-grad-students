...
for item in ssh rsync curl node; do command -v "$item" >/dev/null 2>&1 || { echo "ERROR: missing $item" >&2; exit 1; }; done
cd "$site_root"
...
rsync -avz --delete -e "ssh -F $ssh_config_file -o StrictHostKeyChecking=accept-new -o BatchMode=yes -o ConnectTimeout=$connect_timeout" --exclude '.git' ./ "$remote_host:$release_dir/"
